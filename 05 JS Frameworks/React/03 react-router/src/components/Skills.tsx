...
export default class Skills extends React.Component<SkillProps, SkillState> {
  ...
  constructor(props: SkillProps){
    super(props);    
    this.state = {skills :  [{id: 1, name: 'node.js'}, {id: 2, name: 'type script'}, {id: 3, name: 'pnp core js'}], addSkill: ""};
  ...
  render() {
    return (
      <div className="container">
        <div>Your need the follwowing skills:
          <div >
            <label className="lblNewSkill">Enter a new skill:</label>
            <input type="text" value={this.state.addSkill} onChange={this.handleSkillChange} /><button onClick={()=>this.addSkill()}>Add</button>
          </div>
        ...
        <ul>
            {
                this.state.skills.map( (item:Skill) =>{               
                 return <li key={item.id} onClick={()=>this.removeSkill(item) } className="li-skills" >{item.name}</li>
            })}            
        </ul><br/>
  ...
  addSkill() : void {
    ...
    this.setState({skills: this.state.skills.concat([{id: newid, name: this.state.addSkill }])})
